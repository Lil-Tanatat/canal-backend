...
function ConfirmModal({ item, nextState, onConfirm, onCancel, onViewInventory }) {
  ...
  const [urgency, setUrgency]         = useState(item.urgency || "medium");
  ...
  const handleConfirm = () => {
    ...
    onConfirm({ note, qty: parseInt(qty) || 0, damage, repairDetail, photo, urgency, deadline });
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        {isReporting && (<>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={lbl}>ความเร่งด่วน</label>
            <div style={{ display: "flex", gap: 8 }}>
              {[{ v: "high", label: "🔴 เร่งด่วน", color: "#dc2626", bg: "#fee2e2" },
                { v: "medium", label: "🟡 ด่วนเล็กน้อย", color: "#d97706", bg: "#fef3c7" },
                { v: "low", label: "🟢 ไม่ด่วน", color: "#16a34a", bg: "#dcfce7" }].map(u => (
                <button key={u.v} type="button" onClick={() => setUrgency(u.v)} style={{
                  flex: 1, padding: "8px 4px", borderRadius: 8, fontSize: 12, fontWeight: 700,
                  border: `2px solid ${urgency === u.v ? u.color : "#e2e8f0"}`,
                  background: urgency === u.v ? u.bg : "#fff",
                  color: urgency === u.v ? u.color : "#94a3b8", cursor: "pointer",
                }}>{u.label}</button>
              ))}
            </div>
          </div>
          <div style={{ marginBottom: 12 }}>
            <label style={lbl}>กำหนดเสร็จ (ไม่บังคับ)</label>
            <input type="date" value={deadline} onChange={e => setDeadline(e.target.value)}
              min={new Date().toISOString().slice(0,10)}
              style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1.5px solid #e2e8f0", fontSize: 14, fontFamily: "inherit", outline: "none" }} />
          </div>
          <div style={{ marginBottom: 12 }}>
        ...
        {isAccepting && (<>
          ...
          {/* ปรับ urgency/deadline ได้ตอนรับงาน */}
          <div style={{ marginBottom: 12 }}>
            <label style={lbl}>ปรับความเร่งด่วน</label>
            <div style={{ display: "flex", gap: 8 }}>
              {[{ v: "high", label: "🔴 เร่งด่วน", color: "#dc2626", bg: "#fee2e2" },
                { v: "medium", label: "🟡 ด่วนเล็กน้อย", color: "#d97706", bg: "#fef3c7" },
                { v: "low", label: "🟢 ไม่ด่วน", color: "#16a34a", bg: "#dcfce7" }].map(u => (
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={lbl}>ปรับกำหนดเสร็จ</label>
            <input type="date" value={deadline} onChange={e => setDeadline(e.target.value)}
              min={new Date().toISOString().slice(0,10)}
              style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1.5px solid #e2e8f0", fontSize: 14, fontFamily: "inherit", outline: "none" }} />
          </div>
...
export default function ServiceOrder({ user, onLogout, onAdmin, onReport, onInventory, notifBell  }) {
  ...
  const handleConfirm = ({ note, qty, damage, repairDetail, photo, scrapped = false, urgency, deadline }) => {
  ...
  const filtered = items
    ...
    .sort((a, b) => {
      ...
      if (ua !== ub) return ua - ub; // high ก่อน
      // ถ้า urgency เท่ากัน เรียงตาม deadline (ใกล้กำหนดก่อน)
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                <thead>
                  <tr style={{ background: "#f8fafc", borderBottom: "1.5px solid #e2e8f0" }}>
                    {["รหัส", "รูป", "ประเภท", "ชื่อ / รายละเอียด", "สถานที่", "จำนวน", "ด่วน", "สถานะ", ""].map(h => (
                ...
                <tbody>
                  {filtered.length === 0 && (
                    <tr><td colSpan={9} style={{ padding: 32, textAlign: "center", color: "#94a3b8", fontSize: 14 }}>ไม่พบรายการ</td></tr>
                  )}
                      ...
                      <tr key={item.ID + idx} style={{
                        ...
                        <td style={{ padding: "8px 16px" }}>
                          {(() => {
                            ...
                            // ถ้าปกติและไม่มีของพัง แสดง — แทน
                            if (isOk && (!u || u === "medium")) return <span style={{ color: "#cbd5e1", fontSize: 12 }}>—</span>;
                            if (u === "high") return (
                              <div>
                                <span style={{ background: "#fee2e2", color: "#dc2626", fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 99, display: "inline-block" }}>🔴 เร่งด่วน</span>
                                {item.deadline && (() => {
                                  ...
                                  return <div style={{ fontSize: 11, marginTop: 3, color: isOverdue ? "#dc2626" : isSoon ? "#d97706" : "#64748b", fontWeight: isOverdue || isSoon ? 700 : 400 }}>
                                    {isOverdue ? `⚠️ เลย ${Math.abs(diff)} วัน` : isSoon ? `⏰ อีก ${diff} วัน` : `📅 ${d.toLocaleDateString("th-TH")}`}
                                  </div>;
                                })()}
                              </div>
                            );
                            if (u === "medium") return <span style={{ background: "#fef3c7", color: "#d97706", fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 99, display: "inline-block" }}>🟡 ด่วนเล็กน้อย</span>;
                            if (u === "low") return <span style={{ background: "#dcfce7", color: "#16a34a", fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 99, display: "inline-block" }}>🟢 ไม่ด่วน</span>;
