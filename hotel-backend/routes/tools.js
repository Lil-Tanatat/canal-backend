...
router.post('/', requireRole('ช่าง', 'แอดมิน'), upload.single('picture'), async (req, res) => {
  const { ID, Desc, Price, tool_store, place, Date_of_Buy, qty_new, qty_used, qty_damaged, ETC } = req.body;
  // เช็ค ID ซ้ำ
  ...
  await db.query(
    `INSERT INTO tool (ID, \`Desc\`, Picture, Price, tool_store, place, Date_of_Buy, qty_new, qty_used, qty_damaged, ETC)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [ID, Desc, picture, Price || 0, tool_store, place, Date_of_Buy, qty_new || 0, qty_used || 0, qty_damaged || 0, ETC || 0]
  );
...
router.put('/:id', requireRole('ช่าง', 'แอดมิน'), upload.single('picture'), async (req, res) => {
  const { Desc, Price, tool_store, place, Date_of_Buy, qty_new, qty_used, qty_damaged, ETC } = req.body;
  ...
  await db.query(
    ...
    [Desc, picture, Price, tool_store, place, Date_of_Buy, qty_new || 0, qty_used || 0, qty_damaged || 0, ETC, req.params.id]
  );
  res.json({ ok: true });
});
